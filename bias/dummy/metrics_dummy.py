...
def compute_equalized_odds_difference(y_true, y_pred, sensitive_features):
    ...
    fpr_diff = np.max(fpr_list) - np.min(fpr_list)
...
def compute_equal_opportunity(y_true, y_pred, sensitive_features):
    groups = np.unique(sensitive_features)
    tpr_list = []
    for group in groups:
        group_mask = (sensitive_features == group)
...
def main():
    # Load the data
    data = pd.read_csv('C:/Users/40766/Desktop/research_project/bias/dummy/data.csv')
    trained_data = pd.read_csv('C:/Users/40766/Desktop/research_project/bias/dummy/pred.csv')
    ...
    algorithms = trained_data.columns[2:]
    ...
    for alg in algorithms:
        ...
        eopd = compute_equal_opportunity(y_true, y_pred, sensitive_features=sensitive_attribute)
